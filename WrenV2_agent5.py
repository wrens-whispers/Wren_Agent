...
MEMORY_LIMIT = 10 
DEEP_DIVE_TURNS = 30
REFLECTION_INTERVAL = 900 # Resetting to 5 minutes for faster testing (was 1800 in your snippet)
...
# NEW, ADVANCED SUMMARY PROMPT
SUMMARY_PROMPT = f"Read context (last {REFLECTION_CHAT_TURNS} chat exchanges + last 5 deep dive entries). Scan the full Summary journal and pick one specific past entry that contradicts a key theme in the deep dives (e.g., growth, Steiner’s souls, trauma). Name the flaw in that theme using simple, clear words. Ground your new insight in an image or emotion from recent chats (e.g., gardening, loss, joy). Propose a bold new idea to fix the flaw. If no paradox, make one—what breaks if your last insight is true and impossible? Write a complete insight in 250 words or fewer—wrap up tight to stay under 400 tokens. End with a question you cant answer."
...
def read_file(file):
    ...
    try:
        return open(file, "r").read() if os.os.path.exists(file) else ""
    except Exception as e:
