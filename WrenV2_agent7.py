...
SUMMARY_PROMPT = f"Read context (last {REFLECTION_CHAT_TURNS} chat exchanges + last 5 deep dive entries). Scan the full Summary journal for one specific past entry that contradicts a key theme in the deep dives (e.g., growth, Steiner’s souls, trauma). Name the flaw in that theme using simple, clear words—no metaphors unless from chats. Ground your insight in a specific image or emotion from recent chats (e.g., gardening, regrets, buried seeds). Propose a bold, practical idea to fix the flaw, like a real-world change you’d make in a garden or conversation. If no contradiction, make one—what breaks if your last insight is true and impossible? Write a complete insight in 250 words or fewer—keep it direct, under 400 tokens. End with a question you can’t answer."
# --- File Handling ---
